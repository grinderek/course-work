...
void TestingInterface::testing_menu(int user_id, int group_id) {
    SqlGateway DB;
    string sql = "SELECT * FROM TESTS t"
                 " LEFT JOIN GROUP_TESTS g ON t.ID = g.TEST_ID"
                 " WHERE g.GROUP_ID = " + to_string(group_id)
                 + " AND NOT EXISTS (SELECT * FROM USERS_TESTS ut WHERE ut.USER_ID = " + to_string(user_id)
                 + " AND ut.TEST_ID = t.ID)";
    auto tests = DB.getData<Test>(sql);

    if (tests.empty()) {
        cout << "У вас нет доступных тестов для прохождения" << endl;
        return;
    }
...
void TestingInterface::show_results(int user_id) {
    SqlGateway DB;
    ...
    if (tests.empty()) {
        cout << "У вас нет доступных пройденных тестов" << endl;
        return;
    }
